/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Nov 13 13:28:07 2016
 \date		Modified on Sun Nov 13 13:28:07 2016
 \author	
\*//*-@@file@@----------------------------------------------------------------*/
...
PRIVATE SMFRESULT  smfInsertParmData(PSMF pSmf, TICKS tkDelta, LPMIDIHDR lpmh);
...
SMFRESULT  smfOpenFile(PSMFOPENFILESTRUCT psofs)
{
	...
	if (0 == mmioDescend(hmmio, &ckRIFF, NULL, MMIO_FINDRIFF) && ckRIFF.fccType == FOURCC_RMID)
	{
 ...
 *
 *****************************************************************************/
DWORD  smfTicksToMillisecs(HSMF hSmf, TICKS tkOffset)
{
	PSMF pSmf = (PSMF)hSmf;
	...
	if (pSmf->dwTimeDivision & 0x8000)
	{
		...
		dwTicksPerSec = (DWORD)uSMPTE *(DWORD) (BYTE) (pSmf->dwTimeDivision & 0xFF);
	...
	return pTempo->msBase + muldiv32(tkOffset - pTempo->tkTempo, pTempo->dwTempo, 1000L * pSmf->dwTimeDivision);
 ...
 *
 *****************************************************************************/
TICKS  smfMillisecsToTicks(HSMF hSmf, DWORD msOffset)
{
	PSMF pSmf = (PSMF)hSmf;
	PTEMPOMAPENTRY pTempo;
	UINT idx;
	...
	tkOffset = pTempo->tkTempo + muldiv32(msOffset - pTempo->msBase, 1000L * pSmf->dwTimeDivision, pTempo->dwTempo);
...
SMFRESULT  smfReadEvents(HSMF hSmf, LPMIDIHDR lpmh, TICKS tkMax)
{
	...
	while (TRUE)
	{
		...
		if (MIDI_SYSEX > EVENT_TYPE(event))
		{
			...
			*lpdw++ = 0;
			*lpdw++ = (((DWORD)MEVT_SHORTMSG) << 24) | ((DWORD)EVENT_TYPE(event)) |
							(((DWORD)EVENT_CH_B1(event)) << 8) | (((DWORD)EVENT_CH_B2(event)) << 16);

			lpmh->dwBytesRecorded += 3 * sizeof(DWORD);
		}
		else if (MIDI_META == EVENT_TYPE(event) && MIDI_META_EOT == EVENT_META_TYPE(event))
		{
			/* These are ignoreable since smfReadNextEvent()
			 ** takes care of track merging
			 */
		}
		else if (MIDI_META == EVENT_TYPE(event) && MIDI_META_TEMPO == EVENT_META_TYPE(event))
		{
			if (event.cbParm != 3)
			{
			...
			dwTempo = (((DWORD)MEVT_TEMPO) << 24) | (((DWORD)event.hpbParm[0]) << 16) | (((DWORD)event.hpbParm[1]) << 8) | ((DWORD)event.hpbParm[2]);
...
SMFRESULT  smfSeek(HSMF hSmf, TICKS tkPosition, LPMIDIHDR lpmh)
{
		...
		if (MIDI_META == (bEvent = EVENT_TYPE(event)))
		{
			...
			switch (bEvent & 0xF0)
			{
				...
				case MIDI_CONTROLCHANGE:
					kf.rbControl[(((WORD)bEvent & 0x0F) * 120) + EVENT_CH_B1(event)] = EVENT_CH_B2(event);
					break;
			}
	}
	...
	/* Tempo change event?
	 */
	if (KF_EMPTY != kf.rbTempo[0] || KF_EMPTY != kf.rbTempo[1] || KF_EMPTY != kf.rbTempo[2])
	{
		...
		*lpdw++ = 0;
		*lpdw++ = 0;
		*lpdw++ = (((DWORD)kf.rbTempo[0]) << 16) | (((DWORD)kf.rbTempo[1]) << 8) |
									((DWORD)kf.rbTempo[2]) | (((DWORD)MEVT_TEMPO) << 24);
	...
	for (idx = 0; idx < 16; idx++)
	{
		if (KF_EMPTY != kf.rbProgram[idx])
		{
			...
			*lpdw++ = 0;
			*lpdw++ = 0;
			*lpdw++ = (((DWORD)MEVT_SHORTMSG) << 24) | ((DWORD)MIDI_PROGRAMCHANGE) | ((DWORD)idx) | (((DWORD)kf.rbProgram[idx]) << 8);
	...
	for (idxChannel = 0; idxChannel < 16; idxChannel++)
	{
		for (idxController = 0; idxController < 120; idxController++)
		{
			if (KF_EMPTY != kf.rbControl[idx])
			{
				...
				*lpdw++ = 0;
				*lpdw++ = 0;
				*lpdw++ = (((DWORD)MEVT_SHORTMSG << 24) | ((DWORD)MIDI_CONTROLCHANGE) | ((DWORD)idxChannel) | (((DWORD)idxController) << 8) | (((DWORD)kf.rbControl[idx]) << 16));
