/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Nov 13 13:28:20 2016
 \date		Modified on Sun Nov 13 13:28:20 2016
 \author	
\*//*-@@file@@----------------------------------------------------------------*/
 ...
 *
 *****************************************************************************/
SMFRESULT smfBuildFileIndex(PSMF *ppSmf)
{
	SMFRESULT smfrc;
	...
	{
		if (MIDI_META == event.abEvent[0] && MIDI_META_TEMPO == event.abEvent[1])
		{
			if (3 != event.cbParm)
			{
				return SMF_INVALID_FILE;
			...
			else
			{
				 ...
				 */
				pTempo->msBase = (pTempo - 1)->msBase + muldiv32(pTempo->tkTempo - ((pTempo - 1)->tkTempo), (pTempo - 1)->dwTempo, 1000L * pSmf->dwTimeDivision);
			}
			pTempo->dwTempo = (((DWORD)event.hpbParm[0]) << 16) | (((DWORD)event.hpbParm[1]) << 8) | ((DWORD)event.hpbParm[2]);
		}
	}
 ...
 *
 *****************************************************************************/
SMFRESULT smfGetNextEvent(PSMF pSmf, EVENT *pEvent, TICKS tkMax)
{
	PTRACK pTrk;
	PTRACK pTrkFound;
	DWORD idxTrack;
 ...
 *  
 *****************************************************************************/
DWORD smfGetVDword(HPBYTE hpbImage, DWORD dwLeft, DWORD *pDw)
{
	BYTE b;
	DWORD dwUsed = 0;
