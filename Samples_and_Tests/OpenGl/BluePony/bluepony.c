...
static float FrontLegPos[3] = { -0.36, -0.324, 0.108 };
...
static float BackLegPos[3] = { 0.684, -0.324, 0.108 };
...
static float EyePos[3] = { -0.702, 0.648, 0.1116 };
...
static void ExtrudePart(int n, GLfloat v[][2], float depth)
{
 ...
 */
static void MakePony(void)
{
	static GLfloat blue[4]  = { 0.1, 0.1, 1.0, 1.0 };
	static GLfloat black[4] = { 0.0, 0.0, 0.0, 1.0 };
	static GLfloat pink[4]  = { 1.0, 0.5, 0.5, 1.0 };
	...
	glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, blue);
	ExtrudePart(sizeof(FrontLegVerts) / sizeof(GLfloat) / 2, FrontLegVerts, LegDepth);
	glEndList();

	BackLeg = glGenLists(1);
	glNewList(BackLeg, GL_COMPILE);
	glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, blue);
	ExtrudePart(sizeof(BackLegVerts) / sizeof(GLfloat) / 2, BackLegVerts, LegDepth);
	glEndList();
}
 ...
 */
static void DrawPony(float legAngle)
{
	if (!Body)
	{
		MakePony();
...
{
	GLfloat lightPos[4] = { 1.0, 10.0, 10.0, 0.0 };
