/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Nov 13 13:28:31 2016
 \date		Modified on Sun Nov 13 13:28:31 2016
 \author	
\*//*-@@file@@----------------------------------------------------------------*/
...
PRIVATE char gszFormatHMS[] = "88:88:88";
PRIVATE char gszFormatTicks[] = "888888888";
...
PRIVATE VOID NEAR PASCAL PaintTime(HDC hDC)
{
	...
	if (gnTimeFormat == IDS_TICKS)
	{
		...
		for (ii = 0; ii < 9; ii++)
		{
			if (gbRepaint || szOldTicks[ii] != szWork[ii])
				EmbossedTextOut(hDC, ganPosX[ii], gnPosY, szWork + ii, 1, (COLORREF) - 1, (COLORREF) - 1, 3, 3);
	...
	else
	{
		...
		if (gbRepaint || h != oldh)
		{
			...
			EmbossedTextOut(hDC, ganPosX[HOUR_INDEX], gnPosY, szWork, 2, (COLORREF) - 1, (COLORREF) - 1, 3, 3);
			oldh = h;
		}
		...
		if (gbRepaint || m != oldm)
		{
			...
			EmbossedTextOut(hDC, ganPosX[MIN_SEP_INDEX], gnPosY, szWork, 3, (COLORREF) - 1, (COLORREF) - 1, 3, 3);
			oldm = m;
			fRepaintedMin = TRUE;
		}
		...
		if (gbRepaint || s != olds)
		{
			...
			EmbossedTextOut(hDC, ganPosX[SEC_SEP_INDEX], gnPosY, szWork, 3, (COLORREF) - 1, (COLORREF) - 1, 3, 3);
			olds = s;
			fRepaintedSec = TRUE;
		}
		...
		if (gbRepaint)
			for (ii = 0; ii < cb; ii++)
				if (!isdigit(gszFormatHMS[ii]))
				{
					...
					EmbossedTextOut(hDC, ganPosX[ii], gnPosY, gszFormatHMS + ii, 1, (COLORREF) - 1, (COLORREF) - 1, 3, 3);
				}
	}
...
VOID NEAR PASCAL TWnd_OnSize(HWND hWnd, UINT state, int cx, int cy)
{
	...
	hDC = GetDC(hWnd);
	ghFont = CreateScaledFont(hDC, &grcTWnd, (gnTimeFormat == IDS_TICKS) ? gszFormatTicks : gszFormatHMS, ganPosX, &gnPosY);
	ReleaseDC(hWnd, hDC);
 ...
 * 
 *****************************************************************************/
VOID NEAR PASCAL TWnd_OnCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify)
{
	switch (id)
	{
		case IDM_PLAY:
 ...
 * 
 *****************************************************************************/
VOID NEAR PASCAL TWnd_OnDestroy(HWND hWnd)
{
	if (gnTimerOn)
	{
		KillTimer(hWnd, TIMER_ID);
 ...
 *
 *****************************************************************************/
LRESULT CALLBACK TWnd_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
