/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Nov 13 13:27:53 2016
 \date		Modified on Sun Nov 13 13:27:53 2016
 \author	
\*//*-@@file@@----------------------------------------------------------------*/
...
PRIVATE MMRESULT  XlatSMFErr(SMFRESULT smfrc);
...
MMRESULT  seqPreroll(PSEQ pSeq, LPPREROLL lpPreroll)
{
	...
	if (pSeq->uState != SEQ_S_OPENED && pSeq->uState != SEQ_S_PREROLLED)
		return MCIERR_UNSUPPORTED_FUNCTION;
	...
	if (NULL == pSeq->hmidi)
	{
		...
		if ((mmrc = midiStreamOpen((HMIDISTRM*)&pSeq->hmidi, &uDeviceID, 1, (DWORD)seqMIDICallback, 0, CALLBACK_FUNCTION)) != MMSYSERR_NOERROR)
		{
			pSeq->hmidi = NULL;
		...
		if ((mmrc = midiStreamProperty((HMIDISTRM)pSeq->hmidi, (LPBYTE) & mptd, MIDIPROP_SET | MIDIPROP_TIMEDIV)) != MMSYSERR_NOERROR)
		{
	...
	do
	{
		...
		if (SMF_SUCCESS != smfrc)
		{
			if ((SMF_NO_MEMORY != smfrc) || (cbPrerollBuffer >= 32768L))
			{
...
MMRESULT  seqStop(PSEQ pSeq)
{
	 ...
	 */
	if (SEQ_S_PLAYING != pSeq->uState && SEQ_S_PAUSED != pSeq->uState)
	{
		pSeq->fdwSeq &= ~SEQ_F_WAITING;
		return MMSYSERR_NOERROR;
	}
...
MMRESULT  seqTime(PSEQ pSeq, PTICKS pTicks)
{
	...
	if (SEQ_S_PLAYING != pSeq->uState && SEQ_S_PAUSED != pSeq->uState && SEQ_S_PREROLLING != pSeq->uState && SEQ_S_PREROLLED != pSeq->uState && SEQ_S_OPENED != pSeq->uState)
	{
