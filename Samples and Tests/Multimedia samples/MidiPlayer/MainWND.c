/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Nov 13 13:27:24 2016
 \date		Modified on Sun Nov 13 13:27:24 2016
 \author	
\*//*-@@file@@----------------------------------------------------------------*/
...
PRIVATE char gszFilter[] = "MIDI File (*.MID;*.RMI)\0*.MID;*.RMI\0" "All Files (*.*)\0*.*\0";

PRIVATE char gszDefExtension[] = "MID";
...
PRIVATE VOID InitToolbar(HWND hWnd);
PRIVATE VOID InitStatusBar(HWND hWnd);
PRIVATE VOID ResizeStatusBar(HWND hWnd);
PRIVATE VOID SyncUI(HWND hWnd);
PRIVATE VOID SetOneAction(HWND hWnd, int nMenuID, BOOL fEnable);
PRIVATE VOID AttemptFileOpen(HWND hWnd);
PRIVATE BOOL PrerollAndWait(HWND hWnd);

PRIVATE BOOL MWnd_OnCreate(HWND hWnd, CREATESTRUCT FAR *lpCreateStruct);
PRIVATE VOID MWnd_OnGetMinMaxInfo(HWND hWnd, MINMAXINFO FAR *lpMinMaxInfo);
PRIVATE VOID MWnd_OnSize(HWND hWnd, UINT state, int cx, int cy);
PRIVATE VOID MWnd_OnPaint(HWND hWnd);
PRIVATE VOID MWnd_OnDropFiles(HWND hWnd, HDROP hDrop);
PRIVATE VOID MWnd_OnFileOpen(HWND hWnd);
PRIVATE VOID MWnd_OnCommandToggleChild(HWND hWnd, UINT id);
PRIVATE VOID MWnd_OnCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify);
PRIVATE VOID MWnd_OnDestroy(HWND hWnd);
...
PRIVATE VOID InitToolbar(HWND hWnd)
{
	ghWndToolbar = CreateToolbarEx(hWnd, WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, IDC_TOOLBAR, BITMAP_COUNT, ghInst, IDB_TOOLBAR, gatbButton, BUTTON_COUNT, 0, 0, 0, 0, sizeof(TBBUTTON));
...
PRIVATE VOID InitStatusBar(HWND hWnd)
{
	...
	ghWndStatus = CreateStatusWindow(WS_CHILD | SBARS_SIZEGRIP, "", hWnd, IDC_STATBAR);
	...
	if (ghWndStatus)
	{
		...
		if (hFontStat != (HFONT)NULL && hDC != (HDC)NULL)
		{
			...
			{
				GetTextExtentPoint(hDC, grgszTimeFormats[idx], lstrlen(grgszTimeFormats[idx]), &size);
...
PRIVATE VOID ResizeStatusBar(HWND hWnd)
{
	...
	SendMessage(ghWndStatus, SB_SETPARTS, SB_N_PANES, (DWORD) (INT_PTR) (rnPaneEdge));
}
...
//            assert(0);
	...
	SendMessage(ghWndToolbar, TB_PRESSBUTTON, IDM_PAUSE, fPress);
	...
	EnableMenuItem(GetMenu(hWnd), POS_PLAYTHRU, MF_BYPOSITION | ((wActionFlags & SUI_F_CANSELDEVICE) ? MF_ENABLED : MF_DISABLED));
...
PRIVATE VOID SetOneAction(HWND hWnd, int nMenuID, BOOL fEnable)
{
	EnableMenuItem(GetMenu(hWnd), nMenuID, MF_BYCOMMAND | (fEnable ? MF_ENABLED : MF_DISABLED));

	SendMessage(ghWndToolbar, TB_ENABLEBUTTON, nMenuID, (DWORD)fEnable);
}
...
PRIVATE BOOL MWnd_OnCreate(HWND hWnd, CREATESTRUCT FAR *lpCreateStruct)
{
	...
	for (idx = 0; idx < N_TIME_FORMATS; idx++)
	{
		AppendMenu(hMenuOptions, MF_ENABLED | MF_STRING, IDS_TF_FIRST + idx, grgszTimeFormats[idx]);
	}
	...
	for (idx = 0; idx < cDevs; idx++)
	{
		...
		{
			AppendMenu(hMenuPlayThru, MF_ENABLED | MF_STRING, IDM_DEVICES + idx, moutCaps.szPname);
		}
	}
	...
	ghWndTime = CreateWindow(gszTWndClass, NULL, WS_CHILD, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, hWnd, NULL, lpCreateStruct->hInstance, NULL);
...
PRIVATE VOID MWnd_OnGetMinMaxInfo(HWND hWnd, MINMAXINFO FAR *lpMinMaxInfo)
{
	 ...
	 */
	lpMinMaxInfo->ptMinTrackSize.y = (rc.bottom - rc.top) - (grcTWnd.bottom - grcTWnd.top);
}
...
PRIVATE VOID MWnd_OnSize(HWND hWnd, UINT state, int cx, int cy)
{
	...
	{
		GetWindowRect(hWnd, &rc);
		SetWindowPos(hWnd, (HWND)NULL, 0, 0, rc.right - rc.left + 1, rc.bottom - rc.top + 1 - rcClient.top - rcClient.bottom, SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOZORDER);
	}

	SetWindowPos(ghWndTime, (HWND)NULL, rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top, SWP_NOACTIVATE | SWP_NOZORDER);
...
PRIVATE VOID MWnd_OnCommand(HWND hWnd, int id, HWND hWndCtl, UINT codeNotify)
{
	...
	if (id >= IDS_TF_FIRST && id <= IDS_TF_LAST)
	{
		if (NULL != ghWndStatus)
		{
			...
			SendMessage(ghWndStatus, SB_SETTEXT, SB_PANE_TFMT, (LPARAM)lpstr);
	...
	else if (id >= IDM_DEVMIN && id <= IDM_DEVMAX)
	{
		...
		switch (id)
		{
			...
			case IDM_AUTOPLAY:
				...
				CheckMenuItem(GetMenu(hWnd), IDM_AUTOPLAY, MF_BYCOMMAND | (gbAutoPlay ? MF_CHECKED : MF_UNCHECKED));
				break;
 ...
 *
 *****************************************************************************/
LRESULT CALLBACK MWnd_WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
