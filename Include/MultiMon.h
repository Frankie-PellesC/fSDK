/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Jun 19 17:32:59 2016
 \date		Modified on Sun Jun 19 17:32:59 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
#ifdef UNICODE
		(*(FARPROC *) & g_pfnEnumDisplayDevices = GetProcAddress(hUser32, "EnumDisplayDevicesW")) != NULL && (*(FARPROC *) & g_pfnGetMonitorInfo = g_fMultimonPlatformNT ? GetProcAddress(hUser32, "GetMonitorInfoW") : GetProcAddress(hUser32, "GetMonitorInfoA")) != NULL
#else
		(*(FARPROC *) & g_pfnGetMonitorInfo = GetProcAddress(hUser32, "GetMonitorInfoA")) != NULL && (*(FARPROC *) & g_pfnEnumDisplayDevices = GetProcAddress(hUser32, "EnumDisplayDevicesA")) != NULL
#endif
		)
	{
...
int WINAPI xGetSystemMetrics(int nIndex)
{
	...
	switch (nIndex)
	{
			case SM_CMONITORS case SM_SAMEDISPLAYFORMAT return 1;
			case SM_XVIRTUALSCREEN case SM_YVIRTUALSCREEN return 0;
			case SM_CXVIRTUALSCREEN nIndex = SM_CXSCREEN;
			break;
			case SM_CYVIRTUALSCREEN nIndex = SM_CYSCREEN;
			break;
	}
...
HMONITOR WINAPI xMonitorFromPoint(POINT ptScreenCoords, DWORD dwFlags)
{
	...
	if ((dwFlags & (MONITOR_DEFAULTTOPRIMARY | MONITOR_DEFAULTTONEAREST)) || ((ptScreenCoords.x >= 0) && (ptScreenCoords.x < GetSystemMetrics(SM_CXSCREEN)) && (ptScreenCoords.y >= 0) && (ptScreenCoords.y < GetSystemMetrics(SM_CYSCREEN))))
	{
...
HMONITOR WINAPI xMonitorFromRect(LPCRECT lprcScreenCoords, DWORD dwFlags)
{
	...
	if ((dwFlags & (MONITOR_DEFAULTTOPRIMARY | MONITOR_DEFAULTTONEAREST)) || ((lprcScreenCoords->right > 0) && (lprcScreenCoords->bottom > 0) && (lprcScreenCoords->left < GetSystemMetrics(SM_CXSCREEN)) && (lprcScreenCoords->top < GetSystemMetrics(SM_CYSCREEN))))
	{
		return xPRIMARY_MONITOR;
	}
	return NULL;
...
BOOL WINAPI xGetMonitorInfo(HMONITOR hMonitor, out LPMONITORINFO lpMonitorInfo)
{
	RECT rcWork;
	if (InitMultipleMonitorStubs())
	{
	...
	if ((hMonitor == xPRIMARY_MONITOR) && lpMonitorInfo && (lpMonitorInfo->cbSize >= sizeof(MONITORINFO)) && SystemParametersInfoA(SPI_GETWORKAREA, 0, &rcWork, 0))
	{
...
BOOL WINAPI xEnumDisplayMonitors(HDC hdcOptionalForPainting, LPCRECT lprcEnumMonitorsThatIntersect, MONITORENUMPROC lpfnEnumProc, LPARAM dwData)
{
	RECT rcLimit;
	if (InitMultipleMonitorStubs())
	{
	...
	if (hdcOptionalForPainting)
	{
		...
		{
				default if (!GetDCOrgEx(hdcOptionalForPainting, &ptOrg))
					return FALSE;
				OffsetRect(&rcLimit, -ptOrg.x, -ptOrg.y);
				if (IntersectRect(&rcLimit, &rcLimit, &rcClip) && (!lprcEnumMonitorsThatIntersect || IntersectRect(&rcLimit, &rcLimit, lprcEnumMonitorsThatIntersect)))
				{
					break;
				}
...
BOOL WINAPI xEnumDisplayDevices(PVOID Unused, DWORD iDevNum, out PDISPLAY_DEVICE lpDisplayDevice, DWORD dwFlags)
{
	if (InitMultipleMonitorStubs())
