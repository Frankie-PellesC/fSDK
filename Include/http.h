/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Tue Jun 28 17:59:43 2016
 \date		Modified on Tue Jun 28 17:59:43 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
typedef struct __declspec(align(8)) _HTTP_DATA_CHUNK
{
...
HTTPAPI_LINKAGE ULONG WINAPI HttpCreateHttpHandle(PHANDLE pReqQueueHandle, ULONG Reserved);
#if _WIN32_WINNT >= 0x0600
...
#endif
HTTPAPI_LINKAGE ULONG WINAPI HttpAddUrl(HANDLE ReqQueueHandle, PCWSTR pFullyQualifiedUrl, PVOID pReserved);
HTTPAPI_LINKAGE ULONG WINAPI HttpRemoveUrl(HANDLE ReqQueueHandle, PCWSTR pFullyQualifiedUrl);
#if _WIN32_WINNT >= 0x0600
...
#if _WIN32_WINNT >= 0x0600
HTTPAPI_LINKAGE ULONG WINAPI HttpSendHttpResponse(HANDLE ReqQueueHandle, HTTP_REQUEST_ID RequestId, ULONG Flags, PHTTP_RESPONSE pHttpResponse, PHTTP_CACHE_POLICY pCachePolicy, PULONG pBytesSent, PVOID pReserved1, ULONG Reserved2, LPOVERLAPPED pOverlapped, PHTTP_LOG_DATA pLogData);
#else
...
#endif
#if _WIN32_WINNT >= 0x0600
HTTPAPI_LINKAGE ULONG WINAPI HttpSendResponseEntityBody(HANDLE ReqQueueHandle, HTTP_REQUEST_ID RequestId, ULONG Flags, USHORT EntityChunkCount, PHTTP_DATA_CHUNK pEntityChunks, PULONG pBytesSent, PVOID pReserved1, ULONG Reserved2, LPOVERLAPPED pOverlapped, PHTTP_LOG_DATA pLogData);
#else
...
#if _WIN32_WINNT >= 0x0600
HTTPAPI_LINKAGE ULONG WINAPI HttpWaitForDisconnectEx(HANDLE ReqQueueHandle, HTTP_CONNECTION_ID ConnectionId, ULONG Reserved, LPOVERLAPPED pOverlapped);
