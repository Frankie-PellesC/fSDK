...
XMFINLINE BOOL XMQuaternionIsIdentity(FXMVECTOR Q)
{
	...
	XMVECTOR vTemp = _mm_cmpeq_ps(Q, CAST_M128(g_XMIdentityR3));
...
XMFINLINE XMVECTOR XMQuaternionMultiply(FXMVECTOR Q1, FXMVECTOR Q2)
{
	...
	Q2X = _mm_mul_ps(Q2X, CAST_M128(ControlWZYX));
	Q2Y = _mm_mul_ps(Q2Y, Q1Shuffle);
	Q1Shuffle = _mm_shuffle_ps(Q1Shuffle, Q1Shuffle, _MM_SHUFFLE(0, 1, 2, 3));
	Q2Y = _mm_mul_ps(Q2Y, CAST_M128(ControlZWXY));
	Q2Z = _mm_mul_ps(Q2Z, Q1Shuffle);
	vResult = _mm_add_ps(vResult, Q2X);
	Q2Z = _mm_mul_ps(Q2Z, CAST_M128(ControlYXWZ));
...
XMFINLINE XMVECTOR XMQuaternionConjugate(FXMVECTOR Q)
{
	...
	XMVECTOR Result = _mm_mul_ps(Q, CAST_M128(NegativeOne3));
	return Result;
#else
#endif
}
XMFINLINE XMVECTOR XMQuaternionInverse(FXMVECTOR Q)
{
	...
	L = XMVector4LengthSq(Q);
	Conjugate = XMQuaternionConjugate(Q);
	Control = XMVectorLessOrEqual(L, CAST_M128(g_XMEpsilon));
...
XMFINLINE XMVECTOR XMQuaternionLn(FXMVECTOR Q)
{
	...
	XMVECTOR Q0 = _mm_and_ps(Q, CAST_M128(g_XMMask3));
	XMVECTOR ControlW = _mm_cmple_ps(QW, CAST_M128(OneMinusEpsilon));
	XMVECTOR vTemp2 = _mm_cmpge_ps(QW, CAST_M128(NegOneMinusEpsilon));
...
XMFINLINE XMVECTOR XMQuaternionExp(FXMVECTOR Q)
{
	...
	Zero = XMVectorZero();
	Control = XMVectorNearEqual(Theta, Zero, CAST_M128(g_XMEpsilon));
	Result = XMVectorSelect(Result, Q, Control);
	Result = _mm_and_ps(Result, CAST_M128(g_XMMask3));
	CosTheta = _mm_and_ps(CosTheta, CAST_M128(g_XMMaskW));
...
XMINLINE XMVECTOR XMQuaternionSlerpV(FXMVECTOR Q0, FXMVECTOR Q1, FXMVECTOR T)
{
	...
	Zero = XMVectorZero();
	Control = XMVectorLess(CosOmega, Zero);
	Sign = XMVectorSelect(CAST_M128(g_XMOne), CAST_M128(g_XMNegativeOne), Control);
	CosOmega = _mm_mul_ps(CosOmega, Sign);
	Control = XMVectorLess(CosOmega, CAST_M128(OneMinusEpsilon));
	SinOmega = _mm_mul_ps(CosOmega, CosOmega);
	SinOmega = _mm_sub_ps(CAST_M128(g_XMOne), SinOmega);
	SinOmega = _mm_sqrt_ps(SinOmega);
	Omega = XMVectorATan2(SinOmega, CosOmega);
	V01 = _mm_shuffle_ps(T, T, _MM_SHUFFLE(2, 3, 0, 1));
	V01 = _mm_and_ps(V01, CAST_M128(MaskXY));
	V01 = _mm_xor_ps(V01, CAST_M128(SignMask2));
	V01 = _mm_add_ps(CAST_M128(g_XMIdentityR0), V01);
...
XMFINLINE XMVECTOR XMQuaternionIdentity(void)
{
#if defined(_XM_NO_INTRINSICS_)
	return g_XMIdentityR3.v;
#elif defined(_XM_SSE_INTRINSICS_)
	return CAST_M128(g_XMIdentityR3);
#else
#endif
}
...
XMFINLINE XMVECTOR XMQuaternionRotationRollPitchYawFromVector(FXMVECTOR Angles)
{
	...
	HalfAngles = _mm_mul_ps(Angles, CAST_M128(g_XMOneHalf));
	XMVectorSinCos(&SinAngles, &CosAngles, HalfAngles);
	P0 = XMVectorPermute(SinAngles, CosAngles, CAST_M128(ControlPitch));
	Y0 = XMVectorPermute(SinAngles, CosAngles, CAST_M128(ControlYaw));
	R0 = XMVectorPermute(SinAngles, CosAngles, CAST_M128(ControlRoll));
	P1 = XMVectorPermute(CosAngles, SinAngles, CAST_M128(ControlPitch));
	Y1 = XMVectorPermute(CosAngles, SinAngles, CAST_M128(ControlYaw));
	R1 = XMVectorPermute(CosAngles, SinAngles, CAST_M128(ControlRoll));
	Q1 = _mm_mul_ps(P1, CAST_M128(Sign));
	Q0 = _mm_mul_ps(P0, Y0);
	Q1 = _mm_mul_ps(Q1, Y1);
	Q0 = _mm_mul_ps(Q0, R0);
	Q = _mm_mul_ps(Q1, R1);
...
XMFINLINE XMVECTOR XMQuaternionRotationNormal(FXMVECTOR NormalAxis, FLOAT Angle)
{
	...
	return Q;
#elif defined(_XM_SSE_INTRINSICS_)
	XMVECTOR N = _mm_and_ps(NormalAxis, CAST_M128(g_XMMask3));
	N = _mm_or_ps(N, CAST_M128(g_XMIdentityR3));
	XMVECTOR Scale = _mm_set_ps1(0.5f * Angle);
	XMVECTOR vSine;
	XMVECTOR vCosine;
	XMVectorSinCos(&vSine, &vCosine, Scale);
	Scale = _mm_and_ps(vSine, CAST_M128(g_XMMask3));
	vCosine = _mm_and_ps(vCosine, CAST_M128(g_XMMaskW));
...
XMFINLINE XMVECTOR XMPlaneDotCoord(FXMVECTOR P, FXMVECTOR V)
{
	...
	return Result;
#elif defined(_XM_SSE_INTRINSICS_)
	XMVECTOR vTemp2 = _mm_and_ps(V, CAST_M128(g_XMMask3));
	vTemp2 = _mm_or_ps(vTemp2, CAST_M128(g_XMIdentityR3));
...
XMFINLINE XMVECTOR XMPlaneIntersectLine(FXMVECTOR P, FXMVECTOR LinePoint1, FXMVECTOR LinePoint2)
{
	...
	Zero = XMVectorZero();
	Control = XMVectorNearEqual(D, Zero, CAST_M128(g_XMEpsilon));
	Result = XMVectorSelect(Point, CAST_M128(g_XMQNaN), Control);
	return Result;
#else
#endif
}
XMINLINE VOID XMPlaneIntersectPlane(XMVECTOR *pLinePoint1, XMVECTOR *pLinePoint2, FXMVECTOR P1, FXMVECTOR P2)
{
	...
	*pLinePoint2 = XMVectorSelect(LinePoint2, g_XMQNaN.v, Control);
	...
	Control = XMVectorLessOrEqual(LengthSq, CAST_M128(g_XMEpsilon));
	*pLinePoint1 = XMVectorSelect(LinePoint1, CAST_M128(g_XMQNaN), Control);
	*pLinePoint2 = XMVectorSelect(LinePoint2, CAST_M128(g_XMQNaN), Control);
#else
#endif
}
...
XMFINLINE XMFLOAT4 *XMPlaneTransformStream(XMFLOAT4 *pOutputStream, UINT OutputStride, CONST XMFLOAT4 *pInputStream, UINT InputStride, UINT PlaneCount, CXMMATRIX M)
{
	return XMVector4TransformStream(pOutputStream, OutputStride, pInputStream, InputStride, PlaneCount, M);
}
XMFINLINE XMVECTOR XMPlaneFromPointNormal(FXMVECTOR Point, FXMVECTOR Normal)
{
	...
	XMVECTOR Result;
	W = XMVector3Dot(Point, Normal);
	W = _mm_mul_ps(W, CAST_M128(g_XMNegativeOne));
	Result = _mm_and_ps(Normal, CAST_M128(g_XMMask3));
	W = _mm_and_ps(W, CAST_M128(g_XMMaskW));
...
XMFINLINE XMVECTOR XMPlaneFromPoints(FXMVECTOR Point1, FXMVECTOR Point2, FXMVECTOR Point3)
{
	...
	N = XMVector3Normalize(N);
	D = XMPlaneDotNormal(N, Point1);
	D = _mm_mul_ps(D, CAST_M128(g_XMNegativeOne));
	N = _mm_and_ps(N, CAST_M128(g_XMMask3));
	D = _mm_and_ps(D, CAST_M128(g_XMMaskW));
...
XMFINLINE XMVECTOR XMColorNegative(FXMVECTOR vColor)
{
	...
	return vResult;
#elif defined(_XM_SSE_INTRINSICS_)
	XMVECTOR vTemp = _mm_xor_ps(vColor, CAST_M128(g_XMNegate3));
	return _mm_add_ps(vTemp, CAST_M128(g_XMOne3));
#else
#endif
}
...
XMFINLINE XMVECTOR XMColorAdjustSaturation(FXMVECTOR vColor, FLOAT fSaturation)
{
	...
	XMVECTOR vLuminance = _mm_mul_ps(vColor, CAST_M128(gvLuminance));
...
XMFINLINE XMVECTOR XMColorAdjustContrast(FXMVECTOR vColor, FLOAT fContrast)
{
	...
	XMVECTOR vResult = _mm_sub_ps(vColor, CAST_M128(g_XMOneHalf));
	vResult = _mm_mul_ps(vResult, vScale);
	vResult = _mm_add_ps(vResult, CAST_M128(g_XMOneHalf));
...
XMINLINE BOOL XMVerifyCPUSupport(void)
{
#if defined(_XM_NO_INTRINSICS_) || !defined(_XM_SSE_INTRINSICS_)
	return TRUE;
#else
...
XMINLINE VOID XMAssert(CONST CHAR *pExpression, CONST CHAR *pFileName, UINT LineNumber)
{
	...
	for (Line = LineNumber, pLineString = aLineString + XMASSERT_LINE_STRING_SIZE - 2; Line != 0 && pLineString >= aLineString; Line /= 10, pLineString--)
	{
...
XMFINLINE XMVECTOR XMFresnelTerm(FXMVECTOR CosIncidentAngle, FXMVECTOR RefractionIndex)
{
	...
	G = _mm_sub_ps(G, CAST_M128(g_XMOne));
	...
	GAddC = _mm_sub_ps(GAddC, CAST_M128(g_XMOne));
	GSubC = _mm_add_ps(GSubC, CAST_M128(g_XMOne));
	GAddC = _mm_mul_ps(GAddC, GAddC);
	GSubC = _mm_mul_ps(GSubC, GSubC);
	GAddC = _mm_div_ps(GAddC, GSubC);
	GAddC = _mm_add_ps(GAddC, CAST_M128(g_XMOne));
	vResult = _mm_mul_ps(vResult, GAddC);
	vResult = _mm_max_ps(vResult, CAST_M128(g_XMZero));
...
XMFINLINE FLOAT XMScalarSinEst(FLOAT Value)
{
	...
	vTemp = _mm_mul_ps(vTemp, CAST_M128(g_XMSinEstCoefficients));
...
XMFINLINE FLOAT XMScalarCosEst(FLOAT Value)
{
	...
	XMVECTOR vTemp = _mm_set_ps(ValueSq * Value, ValueSq, Value, 1.0f);
	vTemp = _mm_mul_ps(vTemp, vTemp);
	vTemp = _mm_mul_ps(vTemp, CAST_M128(g_XMCosEstCoefficients));
	vValue = _mm_shuffle_ps(vValue, vTemp, _MM_SHUFFLE(1, 0, 0, 0));
	vValue = _mm_add_ps(vValue, vTemp);
	vTemp = _mm_shuffle_ps(vTemp, vValue, _MM_SHUFFLE(0, 3, 0, 0));
	vTemp = _mm_add_ps(vTemp, vValue);
...
XMFINLINE VOID XMScalarSinCosEst(FLOAT *pSin, FLOAT *pCos, FLOAT Value)
{
	...
	*pSin = Sin.vector4_f32[0];
	...
	Cos = XMVector4Dot(Cos, CAST_M128(g_XMCosEstCoefficients));
	_mm_store_ss(pCos, Cos);
	Sin = XMVector4Dot(Sin, CAST_M128(g_XMSinEstCoefficients));
...
XMFINLINE FLOAT XMScalarASinEst(FLOAT Value)
{
	...
	*(UINT *) & AbsV = *(UINT *) & Value & 0x7FFFFFFF;
	...
	Result = _mm_mul_ps(Result, CAST_M128(g_XMASinEstCoefficients));
