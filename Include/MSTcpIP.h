/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Aug 14 23:06:15 2016
 \date		Modified on Sun Aug 14 23:06:15 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
{
	return (BOOLEAN) (((a->s_addr & 0x00ff) == 0x0a) || ((a->s_addr & 0xf0ff) == 0x10ac) || ((a->s_addr & 0xffff) == 0xa8c0));
}
...
#define INET_IS_ALIGNED(Pointer, Type) (((ULONG_PTR)Pointer & (_Alignof(Type)-1)) == 0)
MSTCPIP_INLINE SCOPE_LEVEL Ipv4UnicastAddressScope(CONST UCHAR *Address)
{
	...
	if (IN4_IS_ADDR_LINKLOCAL((PIN_ADDR)Address) || IN4_IS_ADDR_LOOPBACK((PIN_ADDR)Address))
	{
...
MSTCPIP_INLINE NL_ADDRESS_TYPE Ipv4AddressType(CONST UCHAR *Address)
{
	...
	if (((Ipv4Address.s_addr & 0x000000ff) == 0) || ((Ipv4Address.s_addr & 0x000000f0) == 240))
	{
		return NlatInvalid;
	}
	return NlatUnicast;
...
{
	return (BOOLEAN) ((Ipv4AddressType((CONST UCHAR *)a) == NlatUnicast) && !(IN4_IS_ADDR_LOOPBACK(a) || IN4_IS_ADDR_LINKLOCAL(a) || IN4_IS_ADDR_SITELOCAL(a) || IN4_IS_ADDR_RFC1918(a)));
}
...
MSTCPIP_INLINE BOOLEAN IN6_PREFIX_EQUAL(CONST IN6_ADDR *a, CONST IN6_ADDR *b, UINT8 len)
{
	...
	return (BOOLEAN) (((memcmp(a, b, Bytes)) == 0) && ((Bits == 0) || ((a->s6_bytes[Bytes] | Mask) == (b->s6_bytes[Bytes] | Mask))));
}
...
{
	return IN6_PREFIX_EQUAL(a, &in6addr_solicitednodemulticastprefix, IN6ADDR_SOLICITEDNODEMULTICASTPREFIX_LENGTH);
}
...
MSTCPIP_INLINE BOOLEAN IN6_IS_ADDR_TEREDO(CONST IN6_ADDR *a)
{
	...
	return (BOOLEAN) (((a->s6_words[0] == in6addr_teredoprefix.s6_words[0]) && (a->s6_words[1] == in6addr_teredoprefix.s6_words[1])) || ((a->s6_words[0] == in6addr_teredoprefix_old.s6_words[0]) && (a->s6_words[1] == in6addr_teredoprefix_old.s6_words[1])));
}
...
MSTCPIP_INLINE SCOPE_LEVEL Ipv6UnicastAddressScope(CONST UCHAR *Address)
{
	...
	if (IN6_IS_ADDR_LINKLOCAL((PIN6_ADDR)Address) || IN6_IS_ADDR_LOOPBACK((PIN6_ADDR)Address))
	{
		return ScopeLevelLink;
	}
...
MSTCPIP_INLINE NL_ADDRESS_TYPE Ipv6AddressType(CONST UCHAR *Address)
{
	...
	if (IN6_IS_ADDR_ISATAP(Ipv6Address) || IN6_IS_ADDR_V4COMPAT(Ipv6Address) || IN6_IS_ADDR_V4MAPPED(Ipv6Address) || IN6_IS_ADDR_V4TRANSLATED(Ipv6Address))
	{
...
MSTCPIP_INLINE BOOLEAN INETADDR_ISEQUAL(CONST SOCKADDR *a, CONST SOCKADDR *b)
{
	if (a->sa_family == AF_INET6)
	{
		return (BOOLEAN) (b->sa_family == AF_INET6 && IN6ADDR_ISEQUAL((CONST SOCKADDR_IN6 *)a, (CONST SOCKADDR_IN6 *)b));
	}
	else
	{
		ASSERT(a->sa_family == AF_INET);
		return (BOOLEAN) (b->sa_family == AF_INET && IN4ADDR_ISEQUAL((CONST SOCKADDR_IN *)a, (CONST SOCKADDR_IN *)b));
	}
}
...
#if (NTDDI_VERSION >= NTDDI_VISTA)
#ifdef _WS2DEF_
NTSYSAPI PSTR NTAPI RtlIpv4AddressToStringA(const struct in_addr *Addr, PSTR S);
NTSYSAPI LONG NTAPI RtlIpv4AddressToStringExA(const struct in_addr *Address, USHORT Port, PSTR AddressString, PULONG AddressStringLength);
NTSYSAPI PWSTR NTAPI RtlIpv4AddressToStringW(const struct in_addr *Addr, PWSTR S);
NTSYSAPI LONG NTAPI RtlIpv4AddressToStringExW(const struct in_addr *Address, USHORT Port, PWSTR AddressString, PULONG AddressStringLength);
NTSYSAPI LONG NTAPI RtlIpv4StringToAddressA(PCSTR S, BOOLEAN Strict, PCSTR *Terminator, struct in_addr *Addr);
NTSYSAPI LONG NTAPI RtlIpv4StringToAddressExA(PCSTR AddressString, BOOLEAN Strict, struct in_addr *Address, PUSHORT Port);
NTSYSAPI LONG NTAPI RtlIpv4StringToAddressW(PCWSTR S, BOOLEAN Strict, LPCWSTR *Terminator, struct in_addr *Addr);
NTSYSAPI LONG NTAPI RtlIpv4StringToAddressExW(PCWSTR AddressString, BOOLEAN Strict, struct in_addr *Address, PUSHORT Port);
#ifdef UNICODE
...
#endif
#ifdef _WS2IPDEF_
NTSYSAPI PSTR NTAPI RtlIpv6AddressToStringA(const struct in6_addr *Addr, PSTR S);
NTSYSAPI LONG NTAPI RtlIpv6AddressToStringExA(const struct in6_addr *Address, ULONG ScopeId, USHORT Port, PSTR AddressString, PULONG AddressStringLength);
NTSYSAPI PWSTR NTAPI RtlIpv6AddressToStringW(const struct in6_addr *Addr, PWSTR S);
NTSYSAPI LONG NTAPI RtlIpv6AddressToStringExW(const struct in6_addr *Address, ULONG ScopeId, USHORT Port, PWSTR AddressString, PULONG AddressStringLength);
NTSYSAPI LONG NTAPI RtlIpv6StringToAddressA(PCSTR S, PCSTR *Terminator, struct in6_addr *Addr);
NTSYSAPI LONG NTAPI RtlIpv6StringToAddressExA(PCSTR AddressString, struct in6_addr *Address, PULONG ScopeId, PUSHORT Port);
NTSYSAPI LONG NTAPI RtlIpv6StringToAddressW(PCWSTR S, PCWSTR *Terminator, struct in6_addr *Addr);
NTSYSAPI LONG NTAPI RtlIpv6StringToAddressExW(PCWSTR AddressString, struct in6_addr *Address, PULONG ScopeId, PUSHORT Port);
#ifdef UNICODE
...
NTSYSAPI PSTR NTAPI RtlEthernetAddressToStringA(const DL_EUI48 *Addr, PSTR S);
NTSYSAPI PWSTR NTAPI RtlEthernetAddressToStringW(const DL_EUI48 *Addr, PWSTR S);
NTSYSAPI LONG NTAPI RtlEthernetStringToAddressA(PCSTR S, PCSTR *Terminator, DL_EUI48 *Addr);
NTSYSAPI LONG NTAPI RtlEthernetStringToAddressW(PCWSTR S, LPCWSTR *Terminator, DL_EUI48 *Addr);
#ifdef UNICODE
