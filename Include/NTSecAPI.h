/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Jul  3 01:18:06 2016
 \date		Modified on Sun Jul  3 01:18:06 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
BOOLEAN NTAPI AuditSetSystemPolicy(PCAUDIT_POLICY_INFORMATION pAuditPolicy, ULONG dwPolicyCount);
BOOLEAN NTAPI AuditSetPerUserPolicy(const PSID pSid, PCAUDIT_POLICY_INFORMATION pAuditPolicy, ULONG dwPolicyCount);
BOOLEAN NTAPI AuditQuerySystemPolicy(const GUID *pSubCategoryGuids, ULONG dwPolicyCount, PAUDIT_POLICY_INFORMATION *ppAuditPolicy);
BOOLEAN NTAPI AuditQueryPerUserPolicy(const PSID pSid, const GUID *pSubCategoryGuids, ULONG dwPolicyCount, PAUDIT_POLICY_INFORMATION *ppAuditPolicy);
BOOLEAN NTAPI AuditEnumeratePerUserPolicy(PPOLICY_AUDIT_SID_ARRAY *ppAuditSidArray);
BOOLEAN NTAPI AuditComputeEffectivePolicyBySid(const PSID pSid, const GUID *pSubCategoryGuids, ULONG dwPolicyCount, PAUDIT_POLICY_INFORMATION *ppAuditPolicy);
BOOLEAN NTAPI AuditComputeEffectivePolicyByToken(HANDLE hTokenHandle, const GUID *pSubCategoryGuids, ULONG dwPolicyCount, PAUDIT_POLICY_INFORMATION *ppAuditPolicy);
BOOLEAN NTAPI AuditEnumerateCategories(GUID **ppAuditCategoriesArray, PULONG pdwCountReturned);
BOOLEAN NTAPI AuditEnumerateSubCategories(const GUID *pAuditCategoryGuid, BOOLEAN bRetrieveAllSubCategories, GUID **ppAuditSubCategoriesArray, PULONG pdwCountReturned);
BOOLEAN NTAPI AuditLookupCategoryNameW(const GUID *pAuditCategoryGuid, PWSTR *ppszCategoryName);
BOOLEAN NTAPI AuditLookupCategoryNameA(const GUID *pAuditCategoryGuid, PSTR *ppszCategoryName);
#ifdef UNICODE
#define AuditLookupCategoryName  AuditLookupCategoryNameW
#else
#define AuditLookupCategoryName  AuditLookupCategoryNameA
#endif
BOOLEAN NTAPI AuditLookupSubCategoryNameW(const GUID *pAuditSubCategoryGuid, PWSTR *ppszSubCategoryName);
BOOLEAN NTAPI AuditLookupSubCategoryNameA(const GUID *pAuditSubCategoryGuid, PSTR *ppszSubCategoryName);
#ifdef UNICODE
...
#endif
BOOLEAN NTAPI AuditLookupCategoryIdFromCategoryGuid(const GUID *pAuditCategoryGuid, PPOLICY_AUDIT_EVENT_TYPE pAuditCategoryId);
BOOLEAN NTAPI AuditLookupCategoryGuidFromCategoryId(POLICY_AUDIT_EVENT_TYPE AuditCategoryId, GUID *pAuditCategoryGuid);
BOOLEAN NTAPI AuditSetSecurity(SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor);
BOOLEAN NTAPI AuditQuerySecurity(SECURITY_INFORMATION SecurityInformation, PSECURITY_DESCRIPTOR *ppSecurityDescriptor);
...
#endif
VOID NTAPI AuditFree(PVOID Buffer);
#if (_WIN32_WINNT >= 0x0601)
