/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Aug  7 22:44:09 2016
 \date		Modified on Sun Aug  7 22:44:09 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
typedef VOID(APIENTRY *PMQRECEIVECALLBACK) (HRESULT hrStatus, QUEUEHANDLE hSource, DWORD dwTimeout, DWORD dwAction, MQMSGPROPS *pMessageProps, LPOVERLAPPED lpOverlapped, HANDLE hCursor);
...
HRESULT APIENTRY MQLocateBegin(LPCWSTR lpwcsContext, MQRESTRICTION *pRestriction, MQCOLUMNSET *pColumns, MQSORTSET *pSort, PHANDLE phEnum);
HRESULT APIENTRY MQLocateNext(HANDLE hEnum, DWORD *pcProps, MQPROPVARIANT aPropVar[]);
HRESULT APIENTRY MQLocateEnd(HANDLE hEnum);
HRESULT APIENTRY MQOpenQueue(LPCWSTR lpwcsFormatName, DWORD dwAccess, DWORD dwShareMode, QUEUEHANDLE *phQueue);
HRESULT APIENTRY MQSendMessage(QUEUEHANDLE hDestinationQueue, MQMSGPROPS *pMessageProps, ITransaction *pTransaction);
HRESULT APIENTRY MQReceiveMessage(QUEUEHANDLE hSource, DWORD dwTimeout, DWORD dwAction, MQMSGPROPS *pMessageProps, LPOVERLAPPED lpOverlapped, PMQRECEIVECALLBACK fnReceiveCallback, HANDLE hCursor, ITransaction *pTransaction);
#if(_WIN32_WINNT >= 0x0501)
HRESULT APIENTRY MQReceiveMessageByLookupId(QUEUEHANDLE hSource, ULONGLONG ullLookupId, DWORD dwLookupAction, MQMSGPROPS *pMessageProps, LPOVERLAPPED lpOverlapped, PMQRECEIVECALLBACK fnReceiveCallback, ITransaction *pTransaction);
#endif
...
HRESULT APIENTRY MQGetQueueSecurity(LPCWSTR lpwcsFormatName, SECURITY_INFORMATION RequestedInformation, PSECURITY_DESCRIPTOR pSecurityDescriptor, DWORD nLength, LPDWORD lpnLengthNeeded);
...
HRESULT APIENTRY MQPathNameToFormatName(LPCWSTR lpwcsPathName, LPWSTR lpwcsFormatName, LPDWORD lpdwFormatNameLength);
HRESULT APIENTRY MQHandleToFormatName(QUEUEHANDLE hQueue, LPWSTR lpwcsFormatName, LPDWORD lpdwFormatNameLength);
HRESULT APIENTRY MQInstanceToFormatName(GUID *pGuid, LPWSTR lpwcsFormatName, LPDWORD lpdwFormatNameLength);
#if(_WIN32_WINNT >= 0x0501)
HRESULT APIENTRY MQADsPathToFormatName(LPCWSTR lpwcsADsPath, LPWSTR lpwcsFormatName, LPDWORD lpdwFormatNameLength);
#endif
VOID APIENTRY MQFreeMemory(PVOID pvMemory);
HRESULT APIENTRY MQGetMachineProperties(LPCWSTR lpwcsMachineName, const GUID *pguidMachineId, MQQMPROPS *pQMProps);
HRESULT APIENTRY MQGetSecurityContext(PVOID lpCertBuffer, DWORD dwCertBufferLength, HANDLE *phSecurityContext);
HRESULT APIENTRY MQGetSecurityContextEx(PVOID lpCertBuffer, DWORD dwCertBufferLength, HANDLE *phSecurityContext);
VOID APIENTRY MQFreeSecurityContext(HANDLE hSecurityContext);
HRESULT APIENTRY MQRegisterCertificate(DWORD dwFlags, PVOID lpCertBuffer, DWORD dwCertBufferLength);
...
HRESULT APIENTRY MQGetPrivateComputerInformation(LPCWSTR lpwcsComputerName, MQPRIVATEPROPS *pPrivateProps);
HRESULT APIENTRY MQPurgeQueue(QUEUEHANDLE hQueue);
HRESULT APIENTRY MQMgmtGetInfo(LPCWSTR pComputerName, LPCWSTR pObjectName, MQMGMTPROPS *pMgmtProps);
HRESULT APIENTRY MQMgmtAction(LPCWSTR pComputerName, LPCWSTR pObjectName, LPCWSTR pAction);
HRESULT APIENTRY MQMarkMessageRejected(HANDLE hQueue, ULONGLONG ullLookupId);
HRESULT APIENTRY MQMoveMessage(QUEUEHANDLE hSourceQueue, QUEUEHANDLE hDestinationQueue, ULONGLONG ullLookupId, ITransaction *pTransaction);
#endif
