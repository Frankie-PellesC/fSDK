/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Sun Jun 19 17:16:02 2016
 \date		Modified on Sun Jun 19 17:16:02 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
#if __POCC__ >= 500
#pragma once
...
{
	STDMETHOD(KsGetTime) (THIS_ LONGLONG * Time);
	STDMETHOD(KsSetTime) (THIS_ LONGLONG Time);
	STDMETHOD(KsGetPhysicalTime) (THIS_ LONGLONG * Time);
	STDMETHOD(KsSetPhysicalTime) (THIS_ LONGLONG Time);
	STDMETHOD(KsGetCorrelatedTime) (THIS_ KSCORRELATED_TIME * CorrelatedTime);
	STDMETHOD(KsSetCorrelatedTime) (THIS_ KSCORRELATED_TIME * CorrelatedTime);
	STDMETHOD(KsGetCorrelatedPhysicalTime) (THIS_ KSCORRELATED_TIME * CorrelatedTime);
	STDMETHOD(KsSetCorrelatedPhysicalTime) (THIS_ KSCORRELATED_TIME * CorrelatedTime);
	STDMETHOD(KsGetResolution) (THIS_ KSRESOLUTION * Resolution);
	STDMETHOD(KsGetState) (THIS_ KSSTATE * State);
};
...
{
	STDMETHOD_(HANDLE, KsGetAllocatorHandle) (THIS);
	STDMETHOD_(KSALLOCATORMODE, KsGetAllocatorMode) (THIS);
	STDMETHOD(KsGetAllocatorStatus) (THIS_ PKSSTREAMALLOCATOR_STATUS AllocatorStatus);
	STDMETHOD_(VOID, KsSetAllocatorMode) (THIS_ KSALLOCATORMODE Mode);
};
...
{
	STDMETHOD_(PALLOCATOR_PROPERTIES_EX, KsGetProperties) (THIS);
	STDMETHOD_(VOID, KsSetProperties) (THIS_ PALLOCATOR_PROPERTIES_EX);
	STDMETHOD_(VOID, KsSetAllocatorHandle) (THIS_ HANDLE AllocatorHandle);
	STDMETHOD_(HANDLE, KsCreateAllocatorAndGetHandle) (THIS_ IKsPin * KsPin);
};  
typedef enum {
...
{
	STDMETHOD(KsQueryMediums) (THIS_ __deref_out PKSMULTIPLE_ITEM * MediumList);
	STDMETHOD(KsQueryInterfaces) (THIS_ __deref_out PKSMULTIPLE_ITEM * InterfaceList);
	STDMETHOD(KsCreateSinkPinHandle) (THIS_ KSPIN_INTERFACE & Interface, KSPIN_MEDIUM & Medium);
	STDMETHOD(KsGetCurrentCommunication) (THIS_ _opt KSPIN_COMMUNICATION * Communication, _opt KSPIN_INTERFACE * Interface, _opt KSPIN_MEDIUM * Medium);
	STDMETHOD(KsPropagateAcquire) (THIS);
	STDMETHOD(KsDeliver) (THIS_ IMediaSample * Sample, ULONG Flags);
	...
	STDMETHOD(KsReceiveAllocator) (THIS_ _opt IMemAllocator * MemAllocator);
	STDMETHOD(KsRenegotiateAllocator) (THIS);
	STDMETHOD_(LONG, KsIncrementPendingIoCount) (THIS);
	STDMETHOD_(LONG, KsDecrementPendingIoCount) (THIS);
...
DECLARE_INTERFACE_(IKsInterfaceHandler, IUnknown)
{
    ...
    STDMETHOD(KsProcessMediaSamples)(
        ...
        _ecount(SampleCount) IMediaSample** SampleList,
        out PLONG SampleCount,
         KSIOOPERATION IoOperation,
...
KsResolveRequiredAttributes(
    ...
    _opt PKSMULTIPLE_ITEM Attributes
    );
KSDDKAPI
HRESULT
WINAPI
...
KsSynchronousDeviceControl(
     HANDLE      Handle,
     ULONG       IoControl,
    _bcount_opt(InLength) PVOID   InBuffer,
     ULONG       InLength,
    _bcount_opt(OutLength) PVOID  OutBuffer,
     ULONG       OutLength,
    out_opt PULONG BytesReturned
    );
KSDDKAPI
HRESULT
WINAPI
...
DECLARE_INTERFACE_(IKsPropertySet, IUnknown)
{
    STDMETHOD(Set)(
        THIS_
         REFGUID PropSet,
         ULONG Id,
        _bcount(InstanceLength) LPVOID InstanceData,
         ULONG InstanceLength,
        _bcount(DataLength) LPVOID PropertyData,
         ULONG DataLength
    ) ;
    STDMETHOD(Get)(
        THIS_
