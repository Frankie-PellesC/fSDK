/*+@@file@@----------------------------------------------------------------*//*!
 ...
 \date		Created  on Wed Jul  6 14:39:00 2016
 \date		Modified on Wed Jul  6 14:39:00 2016
 \author	frankie
\*//*-@@file@@----------------------------------------------------------------*/
...
DECLARE_INTERFACE_(IDsAdminCreateObj, IUnknown)
{
	...
	STDMETHOD(Initialize) (THIS_ IADsContainer * pADsContainerObj, IADs * pADsCopySource, LPCWSTR lpszClassName);
	STDMETHOD(CreateModal) (THIS_ HWND hwndParent, IADs ** ppADsObj);
};
#undef  INTERFACE
...
DECLARE_INTERFACE_(IDsAdminNewObj, IUnknown)
{
	...
	STDMETHOD_(ULONG, AddRef) (THIS);
	STDMETHOD_(ULONG, Release) (THIS);
	STDMETHOD(SetButtons) (THIS_ ULONG nCurrIndex, BOOL bValid);
	STDMETHOD(GetPageCounts) (THIS_ LONG * pnTotal, LONG * pnStartIndex);
};
#undef  INTERFACE
...
DECLARE_INTERFACE_(IDsAdminNewObjPrimarySite, IUnknown)
{
	...
	STDMETHOD_(ULONG, AddRef) (THIS);
	STDMETHOD_(ULONG, Release) (THIS);
	STDMETHOD(CreateNew) (THIS_ LPCWSTR pszName);
...
DECLARE_INTERFACE_(IDsAdminNewObjExt, IUnknown)
{
	...
	STDMETHOD_(ULONG, AddRef) (THIS);
	STDMETHOD_(ULONG, Release) (THIS);
	STDMETHOD(Initialize) (THIS_ IADsContainer * pADsContainerObj, IADs * pADsCopySource, LPCWSTR lpszClassName, IDsAdminNewObj * pDsAdminNewObj, LPDSA_NEWOBJ_DISPINFO pDispInfo);
	STDMETHOD(AddPages) (THIS_ LPFNADDPROPSHEETPAGE lpfnAddPage, LPARAM lParam);
	STDMETHOD(SetObject) (THIS_ IADs * pADsObj);
	STDMETHOD(WriteData) (THIS_ HWND hWnd, ULONG uContext);
	STDMETHOD(OnError) (THIS_ HWND hWnd, HRESULT hr, ULONG uContext);
	STDMETHOD(GetSummaryInfo) (THIS_ BSTR * pBstrText);
};
...
DECLARE_INTERFACE_(IDsAdminNotifyHandler, IUnknown)
{
  ...
  STDMETHOD(Begin)(THIS_ ULONG uEvent, IDataObject* pArg1, IDataObject* pArg2, ULONG* puFlags, BSTR* pBstr) ;
  STDMETHOD(Notify)(THIS_ ULONG nItem, ULONG uFlags) ; 
  STDMETHOD(End)(THIS) ; 
};
#endif
